...
Update 13/10: Belen added population numbers for later use in c2num function. 
...
save(gbd_temp, local_government_areas, DISEASE_SHORT_NAMES, file=file.path(relative_path_mslt, "/input/gbd/GBD2019/METAHIT/GBD2019.rda"))
```
...
saveRDS(gbddb, file=file.path(relative_path_mslt, "/input/gbd/GBD2019/METAHIT/gbdbb.rds"))
gbdmod %>% 
...
input_disbayes <- gbddb %>% filter(!str_detect(disease, "injuries")) %>%
  ...
  group_by(disease) %>%
    mutate(inc_exclude=ifelse(sum(inc_num) <= 200, T, F),
          prev_exclude=ifelse(sum(prev_num) <= 200, T, F)) %>%
    filter(inc_exclude == F, prev_exclude == F) %>%
    select(-inc_exclude, -prev_exclude) %>%
  ungroup()

diseases_small <- gbddb %>% select(disease) %>% filter(!gbddb$disease %in% input_disbayes$disease)


saveRDS(input_disbayes,file=file.path(relative_path_mslt, "/input/city regions/Input disbayes/disbayes_input_2_11_21.rds"))
```