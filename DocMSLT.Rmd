...
```{r,eval=FALSE}
source(paste0(relative_path_mslt, "R/run_MSLT.R"))
...
TO DO: CHECK CALCULATIONS OF NON DISEASE PIFS. THIS IS NOT RIGHT AS SOME FIGURES ARE OVER ONE. CALCULATION SCENARIO_NUMBERS/BASELINE_NUMBER?
...
#### Diseases life tables
  ...
  ### Step needed to calculate numbers (rates*people cohort)
    ...
    pivot_wider(names_from  = cause,
                ...
                values_fill = 0) %>%
    dplyr::select(-starts_with(c("Lx", "mx", "px", "incidence_")))
...
```
Organise outputs for presentation
...
```{r, eval=FALSE}
  ...
  output_life_expectancy_change <- output_df[!duplicated(output_df$cohort), c("Age group", "cohort", "Gender", "ex_bl", "ex_sc", "ewx_bl", "ewx_sc", "ex_diff", "ewx_diff", "population_number")] %>%
    ...
    relocate(population_number, .after = Gender)%>%
  ...
  output_life_years_change <- output_df %>% 
    ...
    summarise_if(is.numeric, funs(sum)) %>%
    ungroup() %>%
    dplyr::select(`Age group`,cohort, Gender,Lx_diff, Lwx_diff, population_number) %>%
  ...
  output_diseases_change <- output_df %>% 
    ...
    rename('Population cohort'=population_number) %>%
    dplyr::select(-cohort) %>%
    dplyr::select(`Age group`, Gender, contains("diff"), -contains("Lx", "Lwx", "ex", "exw"))
```